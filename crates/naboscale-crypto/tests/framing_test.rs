use naboscale_crypto::{
    Initiator, Keypair, Responder, Tai64N, INIT_SIZE, RESPONSE_SIZE,
};
...
fn response_message_is_wireguard_compatible_size() {
    ...
    assert_eq!(resp_receiver_id, 1, "receiver_id must echo initiator's sender_id");
}
...
fn responder_rejects_init_with_wrong_message_type() {
    ...
    assert!(result.is_err(), "responder must reject unknown message types");
}
...
fn initiator_rejects_response_with_wrong_receiver_id() {
    ...
    assert!(result.is_err(), "initiator must reject responses not addressed to it");
}
